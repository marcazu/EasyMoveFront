...
public class UpdateMarkerTask extends AsyncTask<String, Void, JSONObject>
{
    ...
    public UpdateMarkerTask (Context context){
        mContext = context;
    }
    ...
    protected JSONObject doInBackground(String... strings)
    {
        ...
        try {
            ...
            if (strings[6].equals("0")) {
                Bitmap pic = CurrentBitmap.getInstance().getBitMap();
                FileOutputStream stream = new FileOutputStream(file);
                pic.compress(Bitmap.CompressFormat.PNG, 100, stream);
                stream.close();
                idFoto = uploadFoto(file);
            }
            if (!idFoto.equals("0") || !strings[6].equals("0")) {

                String fotoUrl = "";
                if (!idFoto.equals("0"))
                    fotoUrl = getFotoFromBack(idFoto);
                else
                    fotoUrl = strings[6];
                ...
                json.put("idUsuariCreador", Integer.parseInt(strings[3]));
                json.put("latitud", Double.valueOf(strings[4]));
                json.put("longitud", Double.valueOf(strings[5]));
                json.put("nom", strings[1]);
    ...
    protected void onPostExecute(JSONObject result) {
        asyncResponse.processFinish(String.valueOf(returnCode));
